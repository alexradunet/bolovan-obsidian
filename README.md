...
The default brain folder is `system/Bolovan` and can be changed in settings. A manifest lets another device auto-discover it. Root `AGENTS.md` instructions, Agent Skills packages, and device-owned conversation branches sync as ordinary visible vault files. Concurrent branches are preserved and never automatically merged.

Bolovan discovers canonical Agent Skills at `Skills/<name>/SKILL.md`. It sends only each valid skill's name and description initially, loads complete instructions when the task or an explicit `[[skill-name]]` mention activates them, and reads bounded package resources only on demand. Skill scripts are source material, never executable capabilities.

Bolovan may propose a skill after a successful non-trivial workflow, a useful correction, or recovery from failure. Creation and updates use the same exact-preview approval as other vault changes, and skills can only describe how to use capabilities Bolovan already has. Invalid or oversized packages are excluded with a visible diagnostic.

The composer can attach the active note and ordinary `[[wikilink]]` mentions. Valid bare skill mentions activate skills instead of attaching their `SKILL.md` files as note data. Paste an HTTP or HTTPS link into a message and Bolovan can fetch it through Obsidian's cross-platform network API, extract readable text, and use that text to answer. Images embedded in an answer with Markdown or a vault image embed display directly in the conversation. Tool calls are visible in the transcript, runs are cancellable, and conversations can be resumed from the session picker.
